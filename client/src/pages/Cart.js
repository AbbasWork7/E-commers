...
const Cart = ({ theme }) => {
  ...
  const totalPrice = cartItems.reduce((total, item) => {
    return total + (item.price || item.sale_price) * item.quantity;
  }, 0);
  ...
  const handleCheckout = () => {
    if (user) {
      alert(`Your user ID is: ${user.user_id}`);
      navigate('/checkout');
    } else {
                  ...
                  <Card.Body>
                    <Row className="align-items-center">
                      ...
                      <Col xs={9} sm={4}>
                        ...
                        <p className="mb-0 text-primary">₹{item.price || item.sale_price}</p>
                      </Col>
                      <Col xs={6} sm={3}>
                      ...
                      <Col xs={6} sm={3} className="text-end">
                        <p className="mb-2 text-primary fw-bold">₹{item.price * item.quantity || item.sale_price * item.quantity}</p>
                        <Button 
              ...
              <Card className={`${theme === 'dark' ? 'bg-dark text-light border-secondary' : ''}`}>
                <Card.Body>
                  ...
                  <Button 
                    variant="primary" 
                    className="w-100"
                    onClick={handleCheckout} // Trigger the alert here
                  >
                    Proceed to Checkout
                  </Button>
                </Card.Body>
